...
class EventsResourceIT {
    ...
    private static final Integer SMALLER_LANE = 1 - 1;

    private static final Integer DEFAULT_DIRECTION = 1;
    private static final Integer UPDATED_DIRECTION = 2;
    private static final Integer SMALLER_DIRECTION = 1 - 1;

    private static final Integer DEFAULT_KPH = 1;
    private static final Integer UPDATED_KPH = 2;
    private static final Integer SMALLER_KPH = 1 - 1;

    private static final Integer DEFAULT_AMBUSH = 1;
    private static final Integer UPDATED_AMBUSH = 2;
    private static final Integer SMALLER_AMBUSH = 1 - 1;
    ...
    private static final Double DEFAULT_WANTED_BY = 1D;
    private static final Double UPDATED_WANTED_BY = 2D;
    private static final Double SMALLER_WANTED_BY = 1D - 1D;
    ...
    private static final Long DEFAULT_RULE_ISSUE = 1L;
    private static final Long UPDATED_RULE_ISSUE = 2L;
    private static final Long SMALLER_RULE_ISSUE = 1L - 1L;
    ...
    private static final String DEFAULT_HANDLED_BY = "AAAAAAAAAA";
    private static final String UPDATED_HANDLED_BY = "BBBBBBBBBB";
    ...
    void createEvents() throws Exception {
        ...
        assertThat(testEvents.getProcessingTime()).isEqualTo(DEFAULT_PROCESSING_TIME);
        ...
        assertThat(testEvents.getThreadRandNo()).isEqualTo(DEFAULT_THREAD_RAND_NO);
    ...
    void getAllEvents() throws Exception {
        ...
        restEventsMockMvc
            ...
            .andExpect(jsonPath("$.[*].kph").value(hasItem(DEFAULT_KPH)))
            .andExpect(jsonPath("$.[*].ambush").value(hasItem(DEFAULT_AMBUSH)))
            ...
            .andExpect(jsonPath("$.[*].wantedBy").value(hasItem(DEFAULT_WANTED_BY.doubleValue())))
            ...
            .andExpect(jsonPath("$.[*].ruleIssue").value(hasItem(DEFAULT_RULE_ISSUE.intValue())))
            ...
            .andExpect(jsonPath("$.[*].handledBy").value(hasItem(DEFAULT_HANDLED_BY)));
    }
    ...
    void getEvents() throws Exception {
        ...
        restEventsMockMvc
            ...
            .andExpect(jsonPath("$.kph").value(DEFAULT_KPH))
            .andExpect(jsonPath("$.ambush").value(DEFAULT_AMBUSH))
            .andExpect(jsonPath("$.toll").value(DEFAULT_TOLL.doubleValue()))
            .andExpect(jsonPath("$.fine").value(DEFAULT_FINE.doubleValue()))
            .andExpect(jsonPath("$.wantedBy").value(DEFAULT_WANTED_BY.doubleValue()))
            ...
            .andExpect(jsonPath("$.ruleIssue").value(DEFAULT_RULE_ISSUE.intValue()))
            ...
            .andExpect(jsonPath("$.handledBy").value(DEFAULT_HANDLED_BY));
    }
    ...
    @Test
    @Transactional
    void getAllEventsByProcessingTimeIsEqualToSomething() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where processingTime equals to DEFAULT_PROCESSING_TIME
        defaultEventsShouldBeFound("processingTime.equals=" + DEFAULT_PROCESSING_TIME);

        // Get all the eventsList where processingTime equals to UPDATED_PROCESSING_TIME
        defaultEventsShouldNotBeFound("processingTime.equals=" + UPDATED_PROCESSING_TIME);
    }

    @Test
    @Transactional
    void getAllEventsByProcessingTimeIsInShouldWork() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where processingTime in DEFAULT_PROCESSING_TIME or UPDATED_PROCESSING_TIME
        defaultEventsShouldBeFound("processingTime.in=" + DEFAULT_PROCESSING_TIME + "," + UPDATED_PROCESSING_TIME);

        // Get all the eventsList where processingTime equals to UPDATED_PROCESSING_TIME
        defaultEventsShouldNotBeFound("processingTime.in=" + UPDATED_PROCESSING_TIME);
    }

    @Test
    @Transactional
    void getAllEventsByProcessingTimeIsNullOrNotNull() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where processingTime is not null
        defaultEventsShouldBeFound("processingTime.specified=true");

        // Get all the eventsList where processingTime is null
        defaultEventsShouldNotBeFound("processingTime.specified=false");
    }

    @Test
    @Transactional
    void getAllEventsByProcessingTimeIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where processingTime is greater than or equal to DEFAULT_PROCESSING_TIME
        defaultEventsShouldBeFound("processingTime.greaterThanOrEqual=" + DEFAULT_PROCESSING_TIME);

        // Get all the eventsList where processingTime is greater than or equal to UPDATED_PROCESSING_TIME
        defaultEventsShouldNotBeFound("processingTime.greaterThanOrEqual=" + UPDATED_PROCESSING_TIME);
    }

    @Test
    @Transactional
    void getAllEventsByProcessingTimeIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where processingTime is less than or equal to DEFAULT_PROCESSING_TIME
        defaultEventsShouldBeFound("processingTime.lessThanOrEqual=" + DEFAULT_PROCESSING_TIME);

        // Get all the eventsList where processingTime is less than or equal to SMALLER_PROCESSING_TIME
        defaultEventsShouldNotBeFound("processingTime.lessThanOrEqual=" + SMALLER_PROCESSING_TIME);
    }

    @Test
    @Transactional
    void getAllEventsByProcessingTimeIsLessThanSomething() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where processingTime is less than DEFAULT_PROCESSING_TIME
        defaultEventsShouldNotBeFound("processingTime.lessThan=" + DEFAULT_PROCESSING_TIME);

        // Get all the eventsList where processingTime is less than UPDATED_PROCESSING_TIME
        defaultEventsShouldBeFound("processingTime.lessThan=" + UPDATED_PROCESSING_TIME);
    }

    @Test
    @Transactional
    void getAllEventsByProcessingTimeIsGreaterThanSomething() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where processingTime is greater than DEFAULT_PROCESSING_TIME
        defaultEventsShouldNotBeFound("processingTime.greaterThan=" + DEFAULT_PROCESSING_TIME);

        // Get all the eventsList where processingTime is greater than SMALLER_PROCESSING_TIME
        defaultEventsShouldBeFound("processingTime.greaterThan=" + SMALLER_PROCESSING_TIME);
    }
    ...
    @Test
    @Transactional
    void getAllEventsByRuleIssueIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where ruleIssue is greater than or equal to DEFAULT_RULE_ISSUE
        defaultEventsShouldBeFound("ruleIssue.greaterThanOrEqual=" + DEFAULT_RULE_ISSUE);

        // Get all the eventsList where ruleIssue is greater than or equal to UPDATED_RULE_ISSUE
        defaultEventsShouldNotBeFound("ruleIssue.greaterThanOrEqual=" + UPDATED_RULE_ISSUE);
    }

    @Test
    @Transactional
    void getAllEventsByRuleIssueIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where ruleIssue is less than or equal to DEFAULT_RULE_ISSUE
        defaultEventsShouldBeFound("ruleIssue.lessThanOrEqual=" + DEFAULT_RULE_ISSUE);

        // Get all the eventsList where ruleIssue is less than or equal to SMALLER_RULE_ISSUE
        defaultEventsShouldNotBeFound("ruleIssue.lessThanOrEqual=" + SMALLER_RULE_ISSUE);
    }

    @Test
    @Transactional
    void getAllEventsByRuleIssueIsLessThanSomething() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where ruleIssue is less than DEFAULT_RULE_ISSUE
        defaultEventsShouldNotBeFound("ruleIssue.lessThan=" + DEFAULT_RULE_ISSUE);

        // Get all the eventsList where ruleIssue is less than UPDATED_RULE_ISSUE
        defaultEventsShouldBeFound("ruleIssue.lessThan=" + UPDATED_RULE_ISSUE);
    }

    @Test
    @Transactional
    void getAllEventsByRuleIssueIsGreaterThanSomething() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where ruleIssue is greater than DEFAULT_RULE_ISSUE
        defaultEventsShouldNotBeFound("ruleIssue.greaterThan=" + DEFAULT_RULE_ISSUE);

        // Get all the eventsList where ruleIssue is greater than SMALLER_RULE_ISSUE
        defaultEventsShouldBeFound("ruleIssue.greaterThan=" + SMALLER_RULE_ISSUE);
    }
    ...
    @Test
    @Transactional
    void getAllEventsByThreadRandNoIsEqualToSomething() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where threadRandNo equals to DEFAULT_THREAD_RAND_NO
        defaultEventsShouldBeFound("threadRandNo.equals=" + DEFAULT_THREAD_RAND_NO);

        // Get all the eventsList where threadRandNo equals to UPDATED_THREAD_RAND_NO
        defaultEventsShouldNotBeFound("threadRandNo.equals=" + UPDATED_THREAD_RAND_NO);
    }

    @Test
    @Transactional
    void getAllEventsByThreadRandNoIsInShouldWork() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where threadRandNo in DEFAULT_THREAD_RAND_NO or UPDATED_THREAD_RAND_NO
        defaultEventsShouldBeFound("threadRandNo.in=" + DEFAULT_THREAD_RAND_NO + "," + UPDATED_THREAD_RAND_NO);

        // Get all the eventsList where threadRandNo equals to UPDATED_THREAD_RAND_NO
        defaultEventsShouldNotBeFound("threadRandNo.in=" + UPDATED_THREAD_RAND_NO);
    }

    @Test
    @Transactional
    void getAllEventsByThreadRandNoIsNullOrNotNull() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where threadRandNo is not null
        defaultEventsShouldBeFound("threadRandNo.specified=true");

        // Get all the eventsList where threadRandNo is null
        defaultEventsShouldNotBeFound("threadRandNo.specified=false");
    }

    @Test
    @Transactional
    void getAllEventsByThreadRandNoContainsSomething() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where threadRandNo contains DEFAULT_THREAD_RAND_NO
        defaultEventsShouldBeFound("threadRandNo.contains=" + DEFAULT_THREAD_RAND_NO);

        // Get all the eventsList where threadRandNo contains UPDATED_THREAD_RAND_NO
        defaultEventsShouldNotBeFound("threadRandNo.contains=" + UPDATED_THREAD_RAND_NO);
    }

    @Test
    @Transactional
    void getAllEventsByThreadRandNoNotContainsSomething() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where threadRandNo does not contain DEFAULT_THREAD_RAND_NO
        defaultEventsShouldNotBeFound("threadRandNo.doesNotContain=" + DEFAULT_THREAD_RAND_NO);

        // Get all the eventsList where threadRandNo does not contain UPDATED_THREAD_RAND_NO
        defaultEventsShouldBeFound("threadRandNo.doesNotContain=" + UPDATED_THREAD_RAND_NO);
    }
    ...
    @Test
    @Transactional
    void getAllEventsByHandledByContainsSomething() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where handledBy contains DEFAULT_HANDLED_BY
        defaultEventsShouldBeFound("handledBy.contains=" + DEFAULT_HANDLED_BY);

        // Get all the eventsList where handledBy contains UPDATED_HANDLED_BY
        defaultEventsShouldNotBeFound("handledBy.contains=" + UPDATED_HANDLED_BY);
    }

    @Test
    @Transactional
    void getAllEventsByHandledByNotContainsSomething() throws Exception {
        // Initialize the database
        eventsRepository.saveAndFlush(events);

        // Get all the eventsList where handledBy does not contain DEFAULT_HANDLED_BY
        defaultEventsShouldNotBeFound("handledBy.doesNotContain=" + DEFAULT_HANDLED_BY);

        // Get all the eventsList where handledBy does not contain UPDATED_HANDLED_BY
        defaultEventsShouldBeFound("handledBy.doesNotContain=" + UPDATED_HANDLED_BY);
    }
    ...
    void putExistingEvents() throws Exception {
        ...
        assertThat(testEvents.getProcessingTime()).isEqualTo(UPDATED_PROCESSING_TIME);
        ...
        assertThat(testEvents.getThreadRandNo()).isEqualTo(UPDATED_THREAD_RAND_NO);
        ...
        await()
            ...
            .untilAsserted(() -> {
                ...
                assertThat(testEventsSearch.getProcessingTime()).isEqualTo(UPDATED_PROCESSING_TIME);
                ...
                assertThat(testEventsSearch.getThreadRandNo()).isEqualTo(UPDATED_THREAD_RAND_NO);
