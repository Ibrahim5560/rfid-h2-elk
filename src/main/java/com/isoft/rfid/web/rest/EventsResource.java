...
import com.isoft.rfid.service.EventsQueryService;
...
/**
 * REST controller for managing {@link com.isoft.rfid.domain.Events}.
 */
@RestController
@RequestMapping("/api")
public class EventsResource {
    ...
    /**
     * {@code SEARCH  /_search/events?query=:query} : search for the events corresponding
     * to the query.
     *
    ...
    public ResponseEntity<List<EventsDTO>> searchEvents(
        ...
        Page<EventsDTO> page = eventsService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
